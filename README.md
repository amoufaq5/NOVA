...
- Enums with variant access (`Op.Add`) — and R17A sum types with payloads (`enum Option { Some(int) None }`, `Option::Some(42)`) plus destructuring match arms (`Option::Some(n) => n`) and compile-time exhaustiveness WARN — R19B extends sum-type lowering to all 6 targets (Linux x86-64, macOS x86-64, Windows x86-64, ARM64-Linux, Windows ARM64, WASM). R20A adds the postfix `?` Result-propagation operator: `let n = parse_num(s)?` unwraps `Result::Ok(n)` to `n` or returns the whole `Result::Err(e)` Result from the enclosing function. The disambiguator from ternary peeks one token past `?`: literals / identifiers / parens / unary / `if` / `match` keep ternary semantics, anything else (binary op, closer, terminator, statement keyword) treats `?` as Result propagation. Const-fold collapses `Result::Ok(constant)?` straight to the constant. R21A adds generic enum payload types: `enum Result<T, E> { Ok(T) Err(E) }` declares a parametric enum, then `Result<int, str>` and `Result<list, int>` annotate `let` bindings — the same decl carries any payload value at construction. Type parameters are erased at codegen (NOVA's tagged-value runtime already handles polymorphic payloads), so the syntax is a zero-cost parser-level metadata layer; `par_skip_type` was extended to handle `>>` (TOK_SHR) for tight-nested generics like `Result<Result<int, str>, str>`. R22B extends parser-only generics from enums to functions: `fn map<T, U>(xs: list<T>, f: T -> U) -> list<U> { ... }` parses generic type parameters between the fn name and the parameter list, accepts `: T` parameter type annotations (legacy), the new `-> T` return-type spelling (Rust-like, alongside the legacy `: T`), and function-type parameters via `T -> U` inside `par_skip_type`. Generic fns compose with R21A generic enums (`fn unwrap<T, E>(r: Result<T, E>) -> T { ... }`). All annotations erased at codegen — same dynamic-runtime tagged-value execution. **R31D** wires the match-arm static analysis into R23A's typecheck pass: `tc_check_match_exhaustive` walks every match node in `tc_walk_stmt` and emits the same `warning: non-exhaustive match on E (missing: V)` line at parse time (rather than only from codegen), then appends a visited-flag to the AST node so the gen_expr + gen_stmt codegen paths de-duplicate the warning to fire EXACTLY ONCE per non-exhaustive site (previously fired 2x). `tc_check_match_arm_types` is a NEW arm-body type-agreement check: arms whose body is a statically recognisable literal (int / str / float / bool / list / map / enum_ctor / struct_init) are compared pairwise, emitting `warning: match arm bodies disagree on type: arm I is T but arm J is U` when the first statically-typed arm and a later one disagree. Conservative — arms whose body type isn't statically known (binders, fn-calls, arithmetic on non-literals) are skipped so the check never false-fires on dynamic values. **R32D** extends pattern syntax in two directions: (1) match arms accept NESTED variant patterns like `match outer { Some(Some(v)) => v, Some(None) => 0, None => -1 }` and mixed enum-in-struct patterns like `match res { Ok(Point { x, y }) => x + y, Err(_) => -1 }` — when a payload slot is itself a sub-pattern the parser emits `AST_NESTED_VARIANT_PATTERN` and rewrites the arm into a guarded wildcard with prep `let binder = tmp[i][j]...` stmts (reusing R25A's struct-pattern rewrite mechanism — no new codegen cases). Single-level patterns continue to emit `AST_ENUM_PATTERN` and flow through R31D's codepath byte-for-byte. (2) `let` binding gains enum-destructure: `let Option::Some(v) = opt` caches the RHS, emits a runtime tag check (`println` + `exit(1)` on mismatch), and binds each payload slot. Multi-variant enums emit a parse-time WARN noting the pattern is refutable; single-variant enums compile silently. Both extensions share `par_enum_tags` — a parser-time table populated by `parse_enum` so discriminant indices resolve before codegen runs. Top-level `let Point { x, y } = p` now registers `x` and `y` as globals via a new `cg_collect_block_globals` helper on all four code generation paths (x86-64, ARM64, Windows ARM64, WASM). **R33C** adds two language ergonomics that both live in parser.nova + codegen.nova: (1) `if let PAT = EXPR { ... } [else ...]` shorthand for the single-arm match `match EXPR { PAT => then, _ => else }` — parses through a new `parse_if_let` helper that mirrors `parse_match`'s pattern dispatch (struct, nested-variant, single-level variant, type-pattern, wildcard, value) and flows through the same R25A / R32D matchee-cache rewrite when the pattern needs it; `else if let` chains by recursive descent; `gen_stmt` learns AST_DO_EXPR so the statement-position form on nested-pattern arms emits code. (2) Match arm guards `Pat if expr => body` add a boolean filter on the arm — the guard evaluates with the pattern's bindings in scope (the R25A / R32D synthetic tag-check chain is split from the user guard so the user guard reads binders inside an `AST_DO_EXPR(prep_stmts, user_guard)` after the tag chain passes; a false guard falls through to the next arm without evaluating the body, never matching more aggressively). Exhaustiveness now runs on the RAW arm list BEFORE the rewrite via `r33c_check_exhaustive_raw` — guarded arms opt out of covering their variant (so `Some(v) if v > 0 => ...` + bare `None => ...` warns missing Some), unguarded `AST_NESTED_VARIANT_PATTERN` still covers its outer variant (preserves R32D byte-identical), and the visited flag on the final match node silences the downstream `tc_check_match_exhaustive` + `cg_check_exhaustive` passes from re-warning. WASM codegen gains guard support on AST_ENUM_PATTERN and generic-value-pattern arms (wildcard guards were already there from R25A).
- **Closure literals (R35C)**: `|x| x + 1` (single-expression body), `|x, y| { let t = x; x + y * t }` (block body), `|| 42` (zero-parameter form) with automatic capture analysis from the enclosing scope. Closures are parser-level syntactic sugar over the existing `fn(params) { body }` lambda — `parse_primary` emits an `AST_LAMBDA` whose body is either a parsed block (when followed by `{`) or a synthetic `{ AST_EXPR_STMT(expr) }` wrapper that flows through gen_function's implicit-return path. Disambiguation vs bitwise OR / logical OR is positional: parse_bitwise and parse_or both call into parse_primary AFTER consuming a left operand, so a leading `|` or `||` in primary position is unambiguously a closure literal. Capture analysis (compute_captures in codegen.nova) walks the body and excludes closure params + body-bound `let` names + builtins + globals; the residue is captured-by-value into static `.bss` slots `_cap_<lambda_N>_<var>` written at the closure-creation site. Global captures behave as live-bindings (closure reads `[rip + _g_<var>]` directly), local captures are by-value at creation. Demonstrated end-to-end with `map_list([1,2,3], |x| x * 2)` → `[2,4,6]`. Limitation: STATIC per-source-position capture slot means multiple instances of the same closure literal (`make_adder(5)` + `make_adder(7)`) share `_cap__lambda_N_k` — last write wins; per-instance capture environments are deferred to a future round.
